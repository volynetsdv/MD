...
float calculate_diou(const GlobalDetection& a, const GlobalDetection& b) {
    ...
    float d2 = (center_a_x - center_b_x) * (center_a_x - center_b_x)
             + (center_a_y - center_b_y) * (center_a_y - center_b_y);
            ...
            for (size_t j = i + 1; j < num_boxes; ++j) {
                ...
                if (!overlap_regions.empty()) {
                    ...
                    for (const auto& r : overlap_regions) {
                        if (box_intersects_region(bi, r)) i_in_overlap = true;
                        if (box_intersects_region(bj, r)) j_in_overlap = true;
                        if (i_in_overlap || j_in_overlap) break;
                    }
