    ...
    void scanDirectory(const std::string& dir) {
        ...
        for (int s : {320, 416, 512, 640}) {
            ...
            for (const std::string& name : {
                ...
                if (f.good()) { f.close(); loadModel(s, dir + "/" + name); break; }
            }
        }
    }
...
static PipelineOptions parse_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if      (a == "--width")       opt.width             = std::atoi(next());
        else if (a == "--height")      opt.height            = std::atoi(next());
        else if (a == "--altitude")    opt.altitude          = static_cast<float>(std::atof(next()));
        else if (a == "--vram-mb")     opt.vram_mb_override  = static_cast<size_t>(std::atoll(next()));
        else if (a == "--max-tiles")   opt.max_tiles         = std::atoi(next());
        else if (a == "--models-dir")  opt.models_dir        = next();
        else if (a == "--engine-320")  opt.engine_320        = next();
        else if (a == "--engine-416")  opt.engine_416        = next();
        else if (a == "--engine-512")  opt.engine_512        = next();
        else if (a == "--engine-640")  opt.engine_640        = next();
        else if (a == "--diou-thresh") opt.diou_thresh       = static_cast<float>(std::atof(next()));
        else if (a == "--conf-thresh") opt.conf_thresh       = static_cast<float>(std::atof(next()));
        else if (a == "--pretty")      opt.pretty_json       = true;
        else if (a == "--verbose")     opt.verbose           = true;
        else if (a == "--help" || a == "-h") {
            std::cerr <<
                "Usage: main_pipeline [options]\n"
                            ...
                            uint8_t* d_external_frame = nullptr)
{
    ...
    if (!opt.engine_320.empty()) pool.loadModel(320, opt.engine_320);
    if (!opt.engine_416.empty()) pool.loadModel(416, opt.engine_416);
    if (!opt.engine_512.empty()) pool.loadModel(512, opt.engine_512);
    if (!opt.engine_640.empty()) pool.loadModel(640, opt.engine_640);
    ...
    if (!detector) {
        ...
        if (owns_frame) cudaFree(d_src_img);
        result.return_code = 1;
        return result;
    }
    ...
    const double ms_per_tile = (result.tiles_processed > 0)
        ? result.loop_ms / static_cast<double>(result.tiles_processed) : 0.0;
